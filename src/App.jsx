...
import ProsPage from "./ProsPage"; import AdminMobile from "./AdminMobile"; import TradePage from "./TradePage";
...
async function fetchApprovedSuppliers() {
  try {
    ...
    return rows.map(v => ({
      id: 2000 + v.id,
      name: v.company,
      category: v.trade || "General", address: v.address || "",       publicAccess: v.public_access === true,
      location:  v.city || "Citrus County, FL",
      website: v.website || "",
      phone: v.phone || "",
      bio: v.bio || ""
...
const SUPPLIERS = [
  { id: 1, name: "Nature Coast Fence Supply", category: "Fencing Materials", location: "Weeki Wachee, FL", website: "https://naturecoastfencesupply.com", phone: "352-710-9991", bio: "Wholesale vinyl, chain link, and aluminum fence materials. Over 90% manufactured in the USA — vinyl fence profiles extruded right in Hernando County. Family-owned since 2018.", address: "11267 Commercial Way, Brooksville, FL 34614", publicAccess: true },
];
...
const TOWNS = [ { name: "Beverly Hills", county: "Citrus", lat: 28.9169, lng: -82.4576 }, { name: "Brooksville", county: "Hernando", lat: 28.5553, lng: -82.3879 }, { name: "Spring Hill", county: "Hernando", lat: 28.4769, lng: -82.5246 }, { name: "Weeki Wachee", county: "Hernando", lat: 28.5225, lng: -82.5732 }, { name: "Citrus Springs", county: "Citrus", lat: 28.9994, lng: -82.4593 }, { name: "Crystal River", county: "Citrus", lat: 28.9025, lng: -82.5926 }, { name: "Dunnellon", county: "Marion", hideInHeader: true, lat: 29.0489, lng: -82.4593 }, { name: "Floral City", county: "Citrus", lat: 28.7494, lng: -82.2965 }, { name: "Hernando", county: "Citrus", lat: 28.9028, lng: -82.3776 }, { name: "Homosassa", county: "Citrus", lat: 28.7811, lng: -82.6134 }, { name: "Homosassa Springs", county: "Citrus", lat: 28.8003, lng: -82.5765 }, { name: "Inverness", county: "Citrus", lat: 28.8358, lng: -82.3304 }, { name: "Lecanto", county: "Citrus", lat: 28.8517, lng: -82.4870 }, { name: "Sugarmill Woods", county: "Citrus", lat: 28.7364, lng: -82.5140 } ];
    ...
    <div className="fade-in"
      ...
      <div style={{ padding: "0 6px" }}>
       ...
       {contractor.claimed === false && (
          ...
          <div style={{ fontSize: 12, color: C.dim, lineHeight: 1.55, marginBottom: 12 }}>We built this page from public record — license and business registration verified. {contractor.company} hasn't added their own details yet.</div><div style={{ maxWidth: 240, margin: "0 auto 10px", borderRadius: 12, overflow: "hidden", border: `1px solid ${C.border}` }}><video src="/bb-tour.mp4" poster="/bb-poster.jpg" controls muted playsInline preload="none" style={{ width: "100%", display: "block", background: C.panel }} /></div><div style={{ fontSize: 11.5, color: C.muted, textAlign: "center", marginBottom: 12 }}>12 seconds — what your page looks like once you claim it</div>
...
function CoverageMap({ results, townObj }) {
  ...
  const layerRef = useRef(null);   const tileRef = useRef(null); const satRef = useRef(null); const [sat, setSat] = useState(true);   const SAT_URL = "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}";   const LBL_URL = "https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}";   const isLight = C.bg === LIGHT.bg;   useEffect(() => { if (!tileRef.current || !mapRef.current) return; if (sat) { tileRef.current.setUrl(SAT_URL); if (!satRef.current) satRef.current = window.L.tileLayer(LBL_URL, { maxZoom: 19 }).addTo(mapRef.current); return; } if (satRef.current) { mapRef.current.removeLayer(satRef.current); satRef.current = null; } tileRef.current.setUrl(`https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_${isLight ? "Light" : "Dark"}_Gray_Base/MapServer/tile/{z}/{y}/{x}`); satRef.current = window.L.tileLayer(`https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_${isLight ? "Light" : "Dark"}_Gray_Reference/MapServer/tile/{z}/{y}/{x}`, { maxZoom: 19 }).addTo(mapRef.current); }, [isLight, sat]);
  ...
  useEffect(() => {
    ...
    if (pts.length > 1) map.fitBounds(L.latLngBounds(pts).pad(0.25), { maxZoom: 11 });
...
useEffect(() => {
 ...
 Promise.all([fetchApprovedVendors(), fetchVendorVideos()]).then(([vendors, videoMap]) => {
  ...
  const [matchSummary, setMatchSummary] = useState("");   const [pabloTurns, setPabloTurns] = useState([]);   const [pabloQuestion, setPabloQuestion] = useState("");   const [pabloAnswer, setPabloAnswer] = useState("");   const [matchOutreach, setMatchOutreach] = useState("");   const [matchQuestions, setMatchQuestions] = useState([]);   const [matchPermit, setMatchPermit] = useState("");   const [matchDanger, setMatchDanger] = useState(false);   const [showAsk, setShowAsk] = useState(false);   const [posting, setPosting] = useState(false);   const [posted, setPosted] = useState(false);   const postJob = async () => { if (!matchOutreach.trim()) { showToast("Nothing to post yet"); return; } setPosting(true); const postTrade = [...new Set((matchResults || []).map(c => tradeBucket(c.trade)))].join(", "); const ok = await submitProjectPost("Project", matchOutreach.trim(), null, postTrade || null); setPosting(false); if (ok) { setPosted(true); track("pablo_post_job", "match"); showToast("Posted — it'll appear once approved"); } else { showToast("Could not post, try again"); } };
  ...
  const openProfile = c => { setActiveProfile(c); setView("profile"); try { window.history.replaceState(null, "", c.id > 1000 ? `/?vendor=${c.id - 1000}` : "/"); } catch (e) {} }; const deepVendor = new URLSearchParams(window.location.search).get("vendor"); useEffect(() => { if (!deepVendor) return; const hit = ALL.find(c => c.id === 1000 + Number(deepVendor)); if (hit) { setActiveProfile(hit); setView("profile"); } }, [deepVendor, ALL]);
  useEffect(() => { try { document.querySelectorAll(".scroll-col").forEach(el => { el.scrollTop = 0; }); window.scrollTo(0, 0); } catch (e) {} }, [view, activeProfile]);
  const goView = id => { setView(id); setActiveProfile(null); try { window.history.replaceState(null, "", "/"); } catch (e) {} };   const [coverageMode, setCoverageMode] = useState(() => new URLSearchParams(window.location.search).get("coverage") === "1");
  const [prosMode, setProsMode] = useState(() => new URLSearchParams(window.location.search).get("pros") === "1" || window.location.pathname.replace(/\/$/, "") === "/pros"); const tradePage = window.location.pathname.replace(/\/$/, "") === "/hvac/citrus-county" ? { trade: "HVAC", county: "Citrus" } : ({ "/roofing/citrus-county": { trade: "Roofing", county: "Citrus" }, "/electrical/citrus-county": { trade: "Electrical", county: "Citrus" }, "/plumbing/citrus-county": { trade: "Plumbing", county: "Citrus" }, "/irrigation/citrus-county": { trade: "Irrigation", county: "Citrus" } })[window.location.pathname.replace(/\/$/, "")] || null;
 ...
 const applyDistance = list => {
  ...
  const runMatch = () => {     if (!matchInput.trim()) return showToast("Describe your project first");     const turns = [{ role: "user", content: matchInput }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const answerPablo = () => {     if (!pabloAnswer.trim()) return showToast("Type your answer first");     const turns = [...pabloTurns, { role: "assistant", content: pabloQuestion }, { role: "user", content: pabloAnswer }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const skipPablo = () => {     const turns = [...pabloTurns, { role: "assistant", content: pabloQuestion }, { role: "user", content: "Skip — just show me contractors." }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const askPablo = async (turns) => {
    setPabloQuestion(""); setMatchOutreach(""); setMatchQuestions([]); setShowAsk(false); setPosted(false); setMatchPermit(""); setMatchDanger(false);
    ...
    try {
      ...
      setMatchSummary(data.summary || ""); setMatchOutreach(data.outreach || ""); setMatchQuestions(data.questions || []); setMatchPermit(data.permit || ""); setMatchDanger(!!data.danger);
    } catch {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{ background: `${C.panel}F0`, backdropFilter: "blur(10px)", borderBottom: `1px solid ${C.border}`, height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 16px", position: "sticky", top: 0, zIndex: 100 }}>
        <button className="menu-btn" onClick={() => setDrawerOpen(true)} aria-label="Open menu" style={{ background: "none", border: "none", cursor: "pointer", color: C.text, padding: 4, marginRight: 4, alignItems: "center" }}><Icon name="menu" size={24} /></button>         <button onClick={() => goView("feed")} style={{ display: "flex", alignItems: "center", gap: 10, background: "none", border: "none", cursor: "pointer", padding: 0 }} aria-label="BuildBridge home" className="logo-btn">
          <svg className="logo-svg" width="150" height="34" viewBox="0 0 900 260" style={{ flexShrink: 0 }}>
<text x="450" y="130" fontFamily="Arial, Helvetica, sans-serif" fontSize="86" fontWeight="700" textAnchor="middle" fill={C.text}>Build<tspan fill={C.orange}>Bridge</tspan></text>
  ...
  <circle cx="700" cy="175" r="7" fill={C.orange} />
        ...
        <div className="top-actions" style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <button onClick={toggleTheme} className="btn-ghost" aria-label={theme === "light" ? "Switch to dark mode" : "Switch to light mode"} title={theme === "light" ? "Dark mode" : "Light mode"} style={{ padding: "8px 11px", fontSize: 15, lineHeight: 1 }}>{theme === "light" ? "🌙" : "☀️"}</button> {authUser ? <button onClick={() => { localStorage.removeItem("bb-token"); setAuthUser(null); showToast("Signed out"); }} className="btn-ghost" style={{ padding: "8px 12px", fontSize: 12.5, fontWeight: 800, color: C.green }}>Signed in — sign out</button> : <button onClick={() => setLoginModal(true)} className="btn-ghost" style={{ padding: "8px 12px", fontSize: 12.5, fontWeight: 800, color: C.dim }}>Contractor login</button>} <button onClick={() => (track("facebook_follow_click", "header"), window.open("https://www.facebook.com/profile.php?id=61591959271748"))} className="btn-ghost fb-btn" aria-label="BuildBridge FL on Facebook" style={{ padding: "8px 12px", fontSize: 12.5, fontWeight: 800, color: C.blue, whiteSpace: "nowrap" }}><span className="fb-stack">Follow us<br/>on Facebook</span><span className="hide-mobile-inline">Follow us on Facebook</span></button> <button onClick={() => (track("facebook_group_click", "header"), window.open("https://www.facebook.com/groups/1454402876451068/"))} className="btn-ghost" aria-label="Citrus County Construction Network Facebook group" style={{ padding: "8px 12px", fontSize: 12.5, fontWeight: 800, color: C.orange }}>Join Group</button> <button onClick={() => setPostModal(true)} className="btn-primary post-btn" style={{ padding: "8px 16px", fontSize: 12.5, display: "flex", alignItems: "center", gap: 6 }}>
            <Icon name="plus" size={14} color="#14100A" /><span className="hide-mobile">Post</span>
          </button>
        </div>
      </header>        {drawerOpen && (         <>           <div className="drawer-backdrop" onClick={() => setDrawerOpen(false)} />           <nav className="drawer" aria-label="Menu">             <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 64, padding: "0 16px", borderBottom: `1px solid ${C.border}` }}>               <div className="display" style={{ fontWeight: 800, fontSize: 16, color: C.white }}>Menu</div>               <button onClick={() => setDrawerOpen(false)} aria-label="Close menu" style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 999, cursor: "pointer", color: C.muted, padding: 6, display: "flex" }}><Icon name="x" size={16} /></button>             </div>             <div style={{ padding: "16px 18px", borderBottom: `1px solid ${C.border}`, background: `${C.orange}0D` }}><div className="eyebrow" style={{ color: C.orange, marginBottom: 6 }}>Keep it in your phone</div><div style={{ fontSize: 12.5, color: C.dim, lineHeight: 1.5, marginBottom: 10 }}>AC out? Pipe burst? Verified pros who cover your town, with their direct number. One tap and BuildBridge FL is in your contacts — send it to a neighbor when they ask.</div><a href={PROS_VCF} download onClick={() => { setDrawerOpen(false); track("pros_vcf_download", "drawer"); }} className="btn-primary" style={{ display: "block", textAlign: "center", width: "100%", padding: 11, fontSize: 13, textDecoration: "none", marginBottom: 8, boxSizing: "border-box" }}>Save to my contacts</a><a href={PROS_URL} onClick={() => { setDrawerOpen(false); track("pros_page_click", "drawer"); }} className="btn-ghost" style={{ display: "block", textAlign: "center", width: "100%", padding: 11, fontSize: 13, textDecoration: "none", boxSizing: "border-box" }}>Open the pros page</a></div>             {DRAWER_SECTIONS.map(sec => (               <div key={sec.heading} style={{ padding: "18px 0 4px", borderBottom: `1px solid ${C.border}` }}>                 <div className="eyebrow" style={{ padding: "0 18px 10px", color: C.muted }}>{sec.heading}</div>                 {sec.items.map(it => (                   <button key={it.id + it.label} onClick={() => { setDrawerOpen(false); goView(it.id); }} style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", background: view === it.id ? `${C.orange}14` : "transparent", border: "none", padding: "12px 18px", cursor: "pointer", color: view === it.id ? C.orange : C.text, fontSize: 14.5, fontWeight: 600, textAlign: "left" }}>                     <Icon name={it.icon} size={19} color={view === it.id ? C.orange : C.muted} />{it.label}                   </button>                 ))}               </div>             ))}             <div style={{ padding: "18px 18px 8px" }}>               <button onClick={() => { setDrawerOpen(false); setPostModal(true); }} className="btn-primary" style={{ width: "100%", padding: 11, fontSize: 13, marginBottom: 8 }}>Share an update</button>               <button onClick={() => { setDrawerOpen(false); authUser ? (localStorage.removeItem("bb-token"), setAuthUser(null), showToast("Signed out")) : setLoginModal(true); }} className="btn-ghost" style={{ width: "100%", padding: 11, fontSize: 13, marginBottom: 8 }}>{authUser ? "Sign out" : "Contractor login"}</button>               <button onClick={() => (track("facebook_follow_click", "drawer"), window.open("https://www.facebook.com/profile.php?id=61591959271748"))} className="btn-ghost" style={{ width: "100%", padding: 11, fontSize: 13, color: C.blue, marginBottom: 8 }}>Follow us on Facebook</button>               <button onClick={() => (track("facebook_group_click", "drawer"), window.open("https://www.facebook.com/groups/1454402876451068/"))} className="btn-ghost" style={{ width: "100%", padding: 11, fontSize: 13, color: C.orange, marginBottom: 8 }}>Join the group</button>               <button onClick={toggleTheme} className="btn-ghost" style={{ width: "100%", padding: 11, fontSize: 13 }}>{theme === "light" ? "🌙  Dark mode" : "☀️  Light mode"}</button>             </div>           </nav>         </>       )}
...
<button onClick={async () => { if (!vApp.name.trim() || !vApp.company.trim() || !vApp.trade.trim() || !vApp.phone.trim()) { showToast("Please fill the required fields (*)"); return; } track("vendor_apply_submit", vApp.company); const ok = await submitVendorApplication(vApp); if (ok) { setVendorModal(false); setVApp({ name: "", company: "", trade: "", phone: "", email: "", website: "", license: "", bio: "" }); showToast("Application sent! We'll verify your business and be in touch."); } else { showToast("Hmm, that didn't send — try again in a minute."); } }} className="btn-primary" style={{ flex: 2, padding: 11, fontSize: 13 }}>Submit application</button>
      ...
      <nav className="bottom-nav" aria-label="Primary mobile">         {MOBILE_TABS.map(n => {           const active = view === n.id || (view === "profile" && n.id === "feed");           return (             <button key={n.id} onClick={() => goView(n.id)} className="nav-btn" aria-label={n.label} aria-current={active ? "page" : undefined}               style={{ flex: 1, background: active ? `${C.orange}18` : "transparent", color: active ? C.orange : C.muted, border: "none", padding: "8px 4px", borderRadius: 10, cursor: "pointer", fontWeight: 700, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>               <Icon name={n.icon} size={21} color={active ? C.orange : C.muted} />               <span style={{ fontSize: 10.5, letterSpacing: "0.03em" }}>{n.label}</span>             </button>           );         })}       </nav>
      ...
      <div className="app-shell">
            ...
            {view === "profile" && activeProfile ? (
             ...
             <>
                ...
                <div style={{ background: `linear-gradient(120deg, ${C.card}, ${C.panel})`, border: `1px solid ${C.border}`, borderRadius: 16, padding: "18px 18px 16px", marginBottom: 18, position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ fontSize: 13, color: C.dim, marginBottom: 14 }}>Describe the job in plain words — our AI matches you with verified contractors near you.</div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "12px 16px", marginBottom: 16 }}>
          ...
          <button className="btn-ghost" onClick={() => (track("get_listed_banner_click", "banner"), setVendorModal(true))} style={{ padding: "9px 14px", fontSize: 12 }}>Get Listed</button>
              ...
              <>
                ...
                {!authUser ? (
                  ...
                  <div key={v.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 14 }}>
                    ...
                    <div onClick={() => setEditForm({ ...editForm, notify_jobs: !editForm.notify_jobs })} style={{ background: C.panel, border: `1px solid ${editForm.notify_jobs ? C.orange + "66" : C.border}`, borderRadius: 10, padding: 14, marginBottom: 14, display: "flex", gap: 12, alignItems: "flex-start", cursor: "pointer" }}><div style={{ width: 18, height: 18, borderRadius: 5, flexShrink: 0, marginTop: 1, border: `2px solid ${editForm.notify_jobs ? C.orange : C.muted}`, background: editForm.notify_jobs ? C.orange : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: "#0b1220", fontSize: 12, fontWeight: 900 }}>{editForm.notify_jobs ? "✓" : ""}</div><div><div style={{ color: C.white, fontWeight: 700, fontSize: 13 }}>Email me new homeowner projects</div><div style={{ color: C.dim, fontSize: 12, lineHeight: 1.5, marginTop: 2 }}>One email each morning, only when there are new posts. Turn it off here anytime.</div></div></div>                     <div style={{ background: C.panel, border: `1px dashed ${C.line}`, borderRadius: 10, padding: 12, marginBottom: 14, fontSize: 12, color: C.dim, lineHeight: 1.5 }}>Business name, trade, license number, and email are locked. Your email is your sign-in — contact BuildBridge to change it.</div>
              ...
              <>
                <SectionHead eyebrow="The directory" title="Contractor network" sub={`Verified local professionals across ${SERVED_LABEL}`} />
                ...
                {filteredNetwork.map(c => (
                  <div key={c.id} className="hover-card" style={{ background: c.claimed !== false ? `linear-gradient(100deg, ${C.gold}0D, ${C.card} 40%)` : C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 14, display: "flex", gap: 14, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                        <div>
                          <button onClick={() => openProfile(c)} style={{ background: "none", border: "none", fontWeight: 800, fontSize: 15, color: C.white, cursor: "pointer", padding: 0, marginBottom: 2, textAlign: "left" }}>{c.name}</button>{c.license && c.license !== "N/A" ? (<div style={{ fontSize: 11.5, fontWeight: 800, color: C.green, margin: "3px 0" }}>🛡 License Verified · <span onClick={e => { e.stopPropagation(); navigator.clipboard?.writeText(String(c.license).split(/[,&]/)[0].trim()); showToast("License copied — pick 'Search by License Number' and paste"); window.open("https://www.myfloridalicense.com/wl11.asp?mode=0&SID=", "_blank"); }} style={{ color: C.green, textDecoration: "underline", cursor: "pointer" }}>{c.license}</span></div>) : c.verified ? (<div style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, margin: "3px 0" }}>✓ Business Verified · Sunbiz</div>) : null}
...
<div style={{ fontSize: 12.5, color: C.dim, lineHeight: 1.6, maxWidth: 420, margin: "0 auto 14px" }}>Contractors: hiring a crew or need a specialty sub? Post it here free and reach every verified pro in {SERVED_LABEL}.</div>
...
{!p.contact && <div style={{ fontSize: 11.5, color: C.dim, marginTop: 8 }}>Interested? Contact BuildBridge to be connected.</div>}
              ...
              <>
                ...
                <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 20, opacity: pabloQuestion ? 0.45 : 1, transition: "opacity 0.25s ease" }}>
                  <textarea value={matchInput} onChange={e => setMatchInput(e.target.value)} aria-label="Describe your project"
                    ...
                    style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: 14, color: C.text, fontSize: 13.5, resize: "vertical", minHeight: 110, fontFamily: "inherit", outline: "none", marginBottom: 14, lineHeight: 1.55 }} />                   <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>                     {["AC not cooling", "Water backing up inside", "Breaker keeps tripping", "Roof leak", "No hot water"].map(p => (                       <button key={p} type="button" onClick={() => setMatchInput(v => (v ? v.trim() + " — " : "") + p)} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 20, padding: "6px 12px", fontSize: 12, color: C.dim, cursor: "pointer" }}>{p}</button>                     ))}                   </div>
                ...
                {pabloQuestion && (                   <div className="fade-in" style={{ background: C.card, border: `1px solid ${C.orange}55`, borderRadius: 16, padding: 20, marginBottom: 20 }}>                     <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 14 }}>                       <Icon name="sparkles" size={16} color={C.orange} style={{ marginTop: 3, flexShrink: 0 }} />                       <div>                         <div style={{ color: C.white, fontWeight: 700, fontSize: 14, marginBottom: 4 }}>Pablo asks</div>                         <div style={{ color: C.dim, fontSize: 14, lineHeight: 1.5 }}>{pabloQuestion}</div>                       </div>                     </div>                     <input value={pabloAnswer} onChange={e => setPabloAnswer(e.target.value)} onKeyDown={e => { if (e.key === "Enter") answerPablo(); }} placeholder="Type your answer…" aria-label="Answer Pablo" style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "11px 13px", color: C.text, fontSize: 14, outline: "none", marginBottom: 10 }} />                     <div style={{ display: "flex", gap: 10 }}>                       <button onClick={answerPablo} className="btn-primary" style={{ flex: 1, padding: 11, fontSize: 13.5 }}>Send answer</button>                       <button onClick={skipPablo} className="btn-ghost" style={{ padding: "11px 16px", fontSize: 13 }}>Skip</button>                     </div>                   </div>                 )}                  {matchResults && (
                  <div className="fade-in">
                    {matchSummary && (
                    ...
                    {matchOutreach && matchResults.length > 0 && (<div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 16 }}><div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}><div><div style={{ color: C.white, fontWeight: 700, fontSize: 13.5 }}>Your message to send</div><div style={{ color: C.muted, fontSize: 11.5, marginTop: 2 }}>Edit anything you like, then copy it into your call, text, or the Message button below</div></div><button onClick={() => { navigator.clipboard.writeText(matchOutreach); showToast("Message copied"); }} className="btn-ghost" style={{ padding: "8px 14px", fontSize: 12.5, whiteSpace: "nowrap" }}>Copy</button></div><textarea value={matchOutreach} onChange={e => setMatchOutreach(e.target.value)} aria-label="Your message to the contractor" style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, color: C.text, fontSize: 13, resize: "vertical", minHeight: 110, fontFamily: "inherit", outline: "none", lineHeight: 1.55 }} /></div>)}                     {matchPermit && matchResults.length > 0 && (<div style={{ background: C.card, border: `1px solid ${C.blue}44`, borderRadius: 12, padding: 14, marginBottom: 16, display: "flex", gap: 11, alignItems: "flex-start" }}><Icon name="clipboard" size={15} color={C.blue} style={{ marginTop: 2, flexShrink: 0 }} /><div><div style={{ color: C.white, fontWeight: 700, fontSize: 13, marginBottom: 3 }}>Worth asking about permits</div><div style={{ color: C.dim, fontSize: 12.5, lineHeight: 1.55 }}>{matchPermit}</div><button onClick={() => setView("permits")} style={{ background: "none", border: "none", color: C.blue, fontSize: 12, fontWeight: 600, padding: "6px 0 0 0", cursor: "pointer" }}>See Permit Prep →</button></div></div>)}                     {matchOutreach && matchResults.length > 0 && (<div style={{ background: C.card, border: `1px solid ${C.orange}44`, borderRadius: 12, padding: 16, marginBottom: 16, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap", justifyContent: "space-between" }}><div style={{ flex: "1 1 260px" }}><div style={{ color: C.white, fontWeight: 700, fontSize: 13.5, marginBottom: 3 }}>{posted ? "Posted to the Projects board" : "Want contractors to come to you?"}</div><div style={{ color: C.dim, fontSize: 12.5, lineHeight: 1.5 }}>{posted ? "It'll appear on the Projects board once we've reviewed it. Your contact details stay private — contractors reach out through BuildBridge." : "I can post this to the Projects board so verified contractors can reach out to you. Your name and contact details stay private."}</div></div><button onClick={postJob} disabled={posting || posted} className={posted ? "btn-ghost" : "btn-primary"} style={{ padding: "10px 18px", fontSize: 13, whiteSpace: "nowrap", opacity: posting || posted ? 0.7 : 1, cursor: posted ? "default" : "pointer" }}>{posted ? "Posted ✓" : posting ? "Posting…" : "Post it for me"}</button></div>)}                     {matchQuestions.length > 0 && matchResults && matchResults.length > 0 && (<div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 16, overflow: "hidden" }}><button onClick={() => setShowAsk(!showAsk)} style={{ width: "100%", background: "none", border: "none", padding: 14, display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", textAlign: "left" }}><div><div style={{ color: C.white, fontWeight: 700, fontSize: 13.5 }}>Before you call</div><div style={{ color: C.muted, fontSize: 11.5, marginTop: 2 }}>{matchQuestions.length} questions worth asking about this job</div></div><span style={{ color: C.orange, fontSize: 18, lineHeight: 1, transform: showAsk ? "rotate(45deg)" : "none", transition: "transform 0.2s ease" }}>+</span></button>{showAsk && (<div style={{ padding: "0 14px 14px 14px" }}>{matchQuestions.map((q, i) => (<div key={i} style={{ display: "flex", gap: 9, alignItems: "flex-start", marginBottom: 9 }}><span style={{ color: C.orange, fontSize: 12, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{i + 1}.</span><span style={{ color: C.dim, fontSize: 13, lineHeight: 1.5 }}>{q}</span></div>))}<div style={{ color: C.muted, fontSize: 11, marginTop: 12, lineHeight: 1.5 }}>Good contractors expect these questions — asking them up front usually saves everyone a second trip.</div></div>)}</div>)}                     {!matchDanger && <div className="eyebrow" style={{ marginBottom: 12 }}>{matchResults.length} matched contractor{matchResults.length !== 1 ? "s" : ""}{matchTimeline === "ASAP" ? " · sorted for fast starts" : ""}</div>}
                    {matchResults.length > 0 && matchResults.map((c, i) => (
          ...
          ) : view === "suppliers" ? (             <>               <SectionHead eyebrow="Vendors & Suppliers" title="Material Suppliers" sub={`Local suppliers ${SERVED_LABEL} contractors trust. Order or reach out directly.`} />                {ALL_SUPPLIERS.map(s => (                 <div key={s.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 16 }}>                   <div className="eyebrow" style={{ marginBottom: 6, color: C.orange }}>{s.category}</div>                   {s.publicAccess === true ? <div style={{ display: "inline-block", fontSize: 10.5, fontWeight: 800, letterSpacing: 0.5, color: C.green, border: `1px solid ${C.green}44`, background: `${C.green}14`, borderRadius: 6, padding: "3px 8px", marginBottom: 8 }}>OPEN TO THE PUBLIC</div> : s.publicAccess === false ? <div style={{ display: "inline-block", fontSize: 10.5, fontWeight: 800, letterSpacing: 0.5, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 6, padding: "3px 8px", marginBottom: 8 }}>TRADE ONLY — ACCOUNT REQUIRED</div> : null}                   <div style={{ fontSize: 18, fontWeight: 800, color: C.white, marginBottom: 6 }}>{s.name}</div>                   <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>{s.location}</div>                   <p style={{ fontSize: 13.5, color: C.dim, lineHeight: 1.6, marginTop: 0, marginBottom: 14 }}>{s.bio}</p>                   <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>                     <a href={`${s.website}?utm_source=buildbridge&utm_medium=supplier_catalog`} target="_blank" rel="noopener" onClick={() => track("supplier_click", s.name)} className="btn-primary" style={{ display: "inline-flex", alignItems: "center", padding: "10px 18px", fontSize: 13, textDecoration: "none" }}>Shop Now</a>                     {s.address && <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(s.address)}`} target="_blank" rel="noopener" onClick={() => track("supplier_directions", s.name)} className="btn-ghost" style={{ padding: "10px 18px", fontSize: 13, textDecoration: "none" }}>Directions</a>}                     {s.phone && <a href={`tel:${s.phone}`} onClick={() => track("supplier_call_click", s.name)} className="btn-ghost" style={{ padding: "10px 18px", fontSize: 13, textDecoration: "none" }}>Call {s.phone}</a>}                   </div>                 </div>               ))}             </>           ) : null}
          </div>
        </main>
        ...
        <aside className="side-col">
          <div className="scroll-col">
            ...
            <div style={{ background: C.card, border: `1px solid ${C.orange}44`, borderRadius: 14, padding: 16, marginBottom: 16, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ fontSize: 13.5, fontWeight: 800, color: C.white, marginBottom: 8 }}>Put your business in front of every verified contractor on the network</div>
