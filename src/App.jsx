        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 16, marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", fontSize: 12.5, color: C.dim }}>
            ...
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
             ...
             {contractor.license === "N/A" ? "License: N/A" : <>License <span onClick={() => { navigator.clipboard?.writeText(String(contractor.license).split(/[,&]/)[0].trim()); showToast("License copied — paste it into DBPR's search box"); window.open("https://www.myfloridalicense.com/wl11.asp?mode=2&search=LicNbr&SID=", "_blank"); }} style={{ color: C.blue, fontWeight: 700, textDecoration: "underline", cursor: "pointer" }}>{contractor.license}</span> · <span onClick={onTrust} style={{ color: C.blue, cursor: "pointer", textDecoration: "underline" }}>verify at FL DBPR</span></>}
                  ...
                  <div key={c.id} className="hover-card" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 14, display: "flex", gap: 14, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                        <div>
                          <button onClick={() => openProfile(c)} style={{ background: "none", border: "none", fontWeight: 800, fontSize: 15, color: C.white, cursor: "pointer", padding: 0, marginBottom: 2, textAlign: "left" }}>{c.name}</button>{c.license && c.license !== "N/A" ? (<div style={{ fontSize: 11.5, fontWeight: 800, color: C.green, margin: "3px 0" }}>🛡 License Verified · <span onClick={e => { e.stopPropagation(); navigator.clipboard?.writeText(String(c.license).split(/[,&]/)[0].trim()); showToast("License copied — paste it into DBPR's search box"); window.open("https://www.myfloridalicense.com/wl11.asp?mode=2&search=LicNbr&SID=", "_blank"); }} style={{ color: C.green, textDecoration: "underline", cursor: "pointer" }}>{c.license}</span></div>) : c.verified ? (<div style={{ fontSize: 11.5, fontWeight: 700, color: C.muted, margin: "3px 0" }}>✓ Business Verified · Sunbiz</div>) : null}
