...
const CONTRACTORS = [
  ...
  { id: 12, name: "Robert Dennis", company: "A & J Junk Removal LLC", verified: true, avatar: "A&J", trade: "Junk Removal & Clean-Outs", location: "Belleview, FL", phone: "(352) 530-3519", email: "aandjjunk82@gmail.com", rating: 5.0, jobs: 0, followers: 0, following: 0, premium: false, license: null, reviews: 0, videoTitle: null, specialties: ["Estate Clean-Outs", "House Clean-Outs", "Yard Debris Removal", "Appliance Removal"], bio: "Family-run junk removal and clean-out service. Estate and house clean-outs, yard debris, and appliance removal. Free estimates. Serving Citrus County and Central Florida." },
  { id: 13, name: "Meghant Purohit", company: "Suncoast Irrigation & Landscaping, LLC", verified: true, avatar: "SI", trade: "Irrigation & Landscaping", location: "Homosassa, FL", phone: "(352) 634-3746", email: "meghant@suncoastirrigationandlandscaping.com", website: "https://suncoastirrigationandlandscaping.com", rating: 5.0, jobs: 0, followers: 0, following: 0, premium: false, license: null, reviews: 0, videoTitle: null, specialties: ["Irrigation Install & Repair", "Smart Sprinkler Controllers", "Landscape Design", "Landscape Lighting", "Palm & Tree Installation"], bio: "Irrigation, landscaping, and outdoor lighting serving Citrus, Hernando, and Marion Counties since 2017. Based in Homosassa." },
];
      ...
      <div className="app-shell">
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap", background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "12px 16px", marginBottom: 16 }}>
          ...
          <button className="btn-ghost" onClick={() => (track("get_listed_banner_click", "banner"), setVendorModal(true))} style={{ padding: "9px 14px", fontSize: 12 }}>Get Listed</button>
            ...
            ) : view === "myprofile" ? (               <>                 <SectionHead eyebrow="Your listing" title="My profile" sub="Edit your BuildBridge listing" />                 {!authUser ? (                   <div style={{ background: C.card, border: `1px dashed ${C.border}`, borderRadius: 16, padding: 30, textAlign: "center", fontSize: 13, color: C.dim }}>Sign in to see your listing.</div>                 ) : myRow === null ? (                   <div style={{ fontSize: 13, color: C.dim }}>Loading…</div>                 ) : myRow.length === 0 ? (                   <div style={{ background: C.card, border: `1px dashed ${C.border}`, borderRadius: 16, padding: 30, textAlign: "center", fontSize: 13, color: C.dim }}>No listing is linked to {authUser.email} yet.</div>                 ) : myRow.map(v => (                   <div key={v.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 14 }}>                     <div style={{ fontSize: 17, fontWeight: 800, color: C.white, marginBottom: 4 }}>{v.company}</div>                     <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 16 }}>{v.name} · {v.trade}</div>                     {[["phone","Phone"],["email","Email"],["website","Website"]].map(([k, label]) => (                       <div key={k} style={{ marginBottom: 10 }}>                         <div className="eyebrow" style={{ fontSize: 10, color: C.muted, marginBottom: 5 }}>{label}</div>                         <input value={editForm[k]} onChange={e => setEditForm({ ...editForm, [k]: e.target.value })} aria-label={label} style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, outline: "none" }} />                       </div>                     ))}                     <div className="eyebrow" style={{ fontSize: 10, color: C.muted, marginBottom: 5 }}>About your business</div>                     <textarea value={editForm.bio} onChange={e => setEditForm({ ...editForm, bio: e.target.value })} aria-label="About your business" style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 12px", color: C.text, fontSize: 13, minHeight: 100, resize: "vertical", fontFamily: "inherit", outline: "none", marginBottom: 14 }} />                     <div style={{ background: C.panel, border: `1px dashed ${C.line}`, borderRadius: 10, padding: 12, marginBottom: 14, fontSize: 12, color: C.dim, lineHeight: 1.5 }}>Business name, trade, and license number are locked. Contact BuildBridge to change those — they're tied to your verification.</div>                     <button onClick={async () => { const token = localStorage.getItem("bb-token"); const ok = await updateMyVendorRow(token, v.id, editForm); showToast(ok ? "Saved" : "Couldn't save — try again"); if (ok) track("profile_self_edit", v.company); }} className="btn-primary" style={{ width: "100%", padding: 12, fontSize: 13.5 }}>Save changes</button>                   </div>                 ))}                 {false && myRow.map(v => (                   <div key={v.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, marginBottom: 14 }}>                     <div style={{ fontSize: 17, fontWeight: 800, color: C.white, marginBottom: 4 }}>{v.company}</div>                     <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 10 }}>{v.name} · {v.trade}</div>                     <div style={{ fontSize: 13, color: C.dim, lineHeight: 1.6 }}>{v.phone}<br />{v.email}</div>                     <div style={{ fontSize: 11.5, color: C.orange, marginTop: 12 }}>Row id {v.id} — editing coming next session</div>                   </div>                 ))}               </>              ) : view === "trust" ? (
              <>
