...
useEffect(() => {
 ...
 Promise.all([fetchApprovedVendors(), fetchVendorVideos()]).then(([vendors, videoMap]) => {
  ...
  const [prosMode, setProsMode] = useState(() => new URLSearchParams(window.location.search).get("pros") === "1" || window.location.pathname.replace(/\/$/, "") === "/pros"); const tradePage = window.location.pathname.replace(/\/$/, "") === "/hvac/citrus-county" ? { trade: "HVAC", county: "Citrus" } : ({ "/roofing/citrus-county": { trade: "Roofing", county: "Citrus" }, "/electrical/citrus-county": { trade: "Electrical", county: "Citrus" }, "/plumbing/citrus-county": { trade: "Plumbing", county: "Citrus" } })[window.location.pathname.replace(/\/$/, "")] || null;
