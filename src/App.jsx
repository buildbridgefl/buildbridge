...
useEffect(() => {
 ...
 Promise.all([fetchApprovedVendors(), fetchVendorVideos()]).then(([vendors, videoMap]) => {
  ...
  const [matchSummary, setMatchSummary] = useState("");   const [pabloTurns, setPabloTurns] = useState([]);   const [pabloQuestion, setPabloQuestion] = useState("");   const [pabloAnswer, setPabloAnswer] = useState("");   const [matchOutreach, setMatchOutreach] = useState("");
 ...
 const applyDistance = list => {
  ...
  const runMatch = () => {     if (!matchInput.trim()) return showToast("Describe your project first");     const turns = [{ role: "user", content: matchInput }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const answerPablo = () => {     if (!pabloAnswer.trim()) return showToast("Type your answer first");     const turns = [...pabloTurns, { role: "assistant", content: pabloQuestion }, { role: "user", content: pabloAnswer }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const skipPablo = () => {     const turns = [...pabloTurns, { role: "assistant", content: pabloQuestion }, { role: "user", content: "Skip — just show me contractors." }];     setPabloTurns(turns);     setPabloAnswer("");     askPablo(turns);   };    const askPablo = async (turns) => {
    setPabloQuestion(""); setMatchOutreach("");
    ...
    try {
      ...
      setMatchSummary(data.summary || ""); setMatchOutreach(data.outreach || "");
    } catch {
              ...
              <>
                ...
                {pabloQuestion && (                   <div className="fade-in" style={{ background: C.card, border: `1px solid ${C.orange}55`, borderRadius: 16, padding: 20, marginBottom: 20 }}>                     <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 14 }}>                       <Icon name="sparkles" size={16} color={C.orange} style={{ marginTop: 3, flexShrink: 0 }} />                       <div>                         <div style={{ color: C.white, fontWeight: 700, fontSize: 14, marginBottom: 4 }}>Pablo asks</div>                         <div style={{ color: C.dim, fontSize: 14, lineHeight: 1.5 }}>{pabloQuestion}</div>                       </div>                     </div>                     <input value={pabloAnswer} onChange={e => setPabloAnswer(e.target.value)} onKeyDown={e => { if (e.key === "Enter") answerPablo(); }} placeholder="Type your answer…" aria-label="Answer Pablo" style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: "11px 13px", color: C.text, fontSize: 14, outline: "none", marginBottom: 10 }} />                     <div style={{ display: "flex", gap: 10 }}>                       <button onClick={answerPablo} className="btn-primary" style={{ flex: 1, padding: 11, fontSize: 13.5 }}>Send answer</button>                       <button onClick={skipPablo} className="btn-ghost" style={{ padding: "11px 16px", fontSize: 13 }}>Skip</button>                     </div>                   </div>                 )}                  {matchResults && (
                  <div className="fade-in">
                    ...
                    {matchOutreach && (<div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 16 }}><div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}><div><div style={{ color: C.white, fontWeight: 700, fontSize: 13.5 }}>Your message to send</div><div style={{ color: C.muted, fontSize: 11.5, marginTop: 2 }}>Edit anything you like, then copy it into your call, text, or the Message button below</div></div><button onClick={() => { navigator.clipboard.writeText(matchOutreach); showToast("Message copied"); }} className="btn-ghost" style={{ padding: "8px 14px", fontSize: 12.5, whiteSpace: "nowrap" }}>Copy</button></div><textarea value={matchOutreach} onChange={e => setMatchOutreach(e.target.value)} aria-label="Your message to the contractor" style={{ width: "100%", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, color: C.text, fontSize: 13, resize: "vertical", minHeight: 110, fontFamily: "inherit", outline: "none", lineHeight: 1.55 }} /></div>)}                     <div className="eyebrow" style={{ marginBottom: 12 }}>{matchResults.length} matched contractor{matchResults.length !== 1 ? "s" : ""}{matchTimeline === "ASAP" ? " · sorted for fast starts" : ""}</div>
