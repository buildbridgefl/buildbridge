...
useEffect(() => {
 ...
 Promise.all([fetchApprovedVendors(), fetchVendorVideos()]).then(([vendors, videoMap]) => {
  ...
  const [prosMode, setProsMode] = useState(() => new URLSearchParams(window.location.search).get("pros") === "1" || window.location.pathname.replace(/\/$/, "") === "/pros"); const tradePage = window.location.pathname.replace(/\/$/, "") === "/hvac/citrus-county" ? { trade: "HVAC", county: "Citrus" } : null;
